             ...
             const OutputPatch<HullOut,3> tri)
{
	...
	dout.ShadowPosH = mul(float4(dout.PosW,1.f), gShadowTransform);
	
	dout.SsaoPosH = mul(float4(dout.PosW,1.f), gViewProjTex);
	return dout; 	
}
		  ...
		  uniform bool gReflectionEnabled) : SV_Target
{
	...
	{  
		...
		float ambientAccess = gSsaoMap.SampleLevel(samLinear, pin.SsaoPosH.xy, 0.f).r;
		[unroll]
