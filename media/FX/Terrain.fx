...
VertexOut VS(VertexIn vin)
{
	...
	//vout.PosL.y = gHeightMap.SampleLevel(samHeightMap, vin.Tex, 0).r;
...
float CalcTessFactor(float3 p)
{
	...
	return pow( 2, ( lerp( gMaxDist, gMinTess, s ) ) );
}
...
PatchTess ConstantHS(InputPatch<VertexOut,4> patch, uint patchID : SV_PrimitiveID )
{
	...
	float3 vMin = float3(patch[2].PosL.x, minY, patch[2].PosL.z);
	float3 vMax = float3(patch[1].PosL.x, maxY, patch[1].PosL.z);
	...
	{
		float3 e0 = 0.5f * ( patch[0].PosL + patch[2].PosL );
		float3 e1 = 0.5f * ( patch[0].PosL + patch[1].PosL );
		float3 e2 = 0.5f * ( patch[1].PosL + patch[3].PosL );
		float3 e3 = 0.5f * ( patch[2].PosL + patch[3].PosL );
		float3 c = 0.5f * ( patch[0].PosL + patch[1].PosL + patch[2].PosL + patch[3].PosL );
...
{
	float2 leftTex = pin.Tex + float2(-gTexelCellSpaceU, 0.f );
	float2 rightTex = pin.Tex + float2(gTexelCellSpaceU, 0.f );
	float2 bottomTex = pin.Tex + float2(0.f, gTexelCellSpaceV);
	float2 topTex = pin.Tex + float2( 0.f, -gTexelCellSpaceV);
	...
	float3 tangent = normalize( float3( 2.f * gWorldCellSpace, rightY - leftY, 0.0f));
