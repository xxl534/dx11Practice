...
void Ssao::BuildFrustumFarCorners(float fovy, float farZ)
{
	...
	mFrustumFarCorner[0] = XMFLOAT4(-halfWidth, -halfHeight, farZ, 0.f);
	mFrustumFarCorner[1] = XMFLOAT4(-halfWidth, +halfHeight, farZ, 0.f);
	mFrustumFarCorner[2] = XMFLOAT4(+halfWidth, +halfHeight, farZ, 0.f);
	mFrustumFarCorner[3] = XMFLOAT4(+halfWidth, -halfHeight, farZ, 0.f);
}
...
void Ssao::BuildFullScreenQuad()
{
	...
	v[0].Pos = XMFLOAT3(-1.f, -1.f, 0.f);
	v[1].Pos = XMFLOAT3(-1.f, 1.f, 0.f);
	v[2].Pos = XMFLOAT3(1.f, 1.f, 0.f);
	v[3].Pos = XMFLOAT3(1.f, -1.f, 0.f);

	//Store far plane frustum corner indices in Normal.x slot.
	v[0].Normal = XMFLOAT3(0.f, 0.f, 0.f);
	v[1].Normal = XMFLOAT3(1.f, 0.f, 0.f);
	v[2].Normal = XMFLOAT3(2.f, 0.f, 0.f);
	v[3].Normal = XMFLOAT3(3.f, 0.f, 0.f);

	v[0].Tex = XMFLOAT2(0.f, 1.f);
	v[1].Tex = XMFLOAT2(0.f, 0.f);
	v[2].Tex = XMFLOAT2(1.f, 0.f);
	v[3].Tex = XMFLOAT2(1.f, 1.f);
...
void Ssao::BuildTextureViews()
{
	...
	ID3D11Texture2D* normalDepthTex = NULL;
	HR(md3dDevice->CreateTexture2D(&texDesc, NULL, &normalDepthTex));
	HR(md3dDevice->CreateShaderResourceView(normalDepthTex, NULL, &mNormalDepthSRV));
	HR(md3dDevice->CreateRenderTargetView(normalDepthTex, NULL, &mNormalDepthRTV));
	...
	ID3D11Texture2D* ambientTex0 = NULL;
	HR(md3dDevice->CreateTexture2D(&texDesc, NULL, &ambientTex0));
	HR(md3dDevice->CreateShaderResourceView(ambientTex0, NULL, &mAmbientSRV0));
	HR(md3dDevice->CreateRenderTargetView(ambientTex0, NULL, &mAmbientRTV0));

	ID3D11Texture2D* ambientTex1 = NULL;
	HR(md3dDevice->CreateTexture2D(&texDesc, NULL, &ambientTex1));
	HR(md3dDevice->CreateShaderResourceView(ambientTex1, NULL, &mAmbientSRV1));
	HR(md3dDevice->CreateRenderTargetView(ambientTex1, NULL, &mAmbientRTV1));
...
void Ssao::BuildRandomVectorTexture()
{
	...
	for (UINT i = 0; i < 256; ++i)
	{
		...
		{
			color[i * 256 + j] = DirectX::PackedVector::XMCOLOR(MathHelper::RandF(), MathHelper::RandF(), MathHelper::RandF(), 0.f);
		}
	}
	...
	HR(md3dDevice->CreateShaderResourceView(tex, NULL, &mRandomVectorSRV));
