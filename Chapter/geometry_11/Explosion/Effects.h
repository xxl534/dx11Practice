...
{
public:
	ExplosionEffect(ID3D11Device* device, const std::wstring& filename);
	~ExplosionEffect();
	...
	void SetMaterial(const Material& mat) { Mat->SetRawValue(&mat, 0, sizeof(Material)); }
	void SetDiffuseMap(ID3D11ShaderResourceView* tex) { DiffuseMap->SetResource(tex); }
	void SetSpeed(float f) { Speed->SetFloat(f); }
	void SetTime(float f) { Time->SetFloat(f); }

	ID3DX11EffectTechnique* TestTech;
	ID3DX11EffectTechnique* Test2Tech;
	...
	ID3DX11EffectScalarVariable* FogStart;
	ID3DX11EffectScalarVariable* FogRange;
	ID3DX11EffectScalarVariable* Speed;
...
public:
	...
	static ExplosionEffect* ExplosionFX;
};
#pragma endregion
