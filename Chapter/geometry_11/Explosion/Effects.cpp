...
#pragma region ExplosionEffect
ExplosionEffect::ExplosionEffect(ID3D11Device* device, const std::wstring& filename)
	: Effect(device, filename)
{
	TestTech = mFX->GetTechniqueByName("Test");
	Test2Tech = mFX->GetTechniqueByName("Test2");
	...
	FogStart = mFX->GetVariableByName("gFogStart")->AsScalar();
	FogRange = mFX->GetVariableByName("gFogRange")->AsScalar();
	Speed = mFX->GetVariableByName("gSpeed")->AsScalar();
...
ExplosionEffect* Effects::ExplosionFX = NULL;

void Effects::InitAll(ID3D11Device* device)
{
	BasicFX = new BasicEffect(device, L"FX/Basic.fx");
	ExplosionFX = new ExplosionEffect(device, L"FX/Explosion.fx");
}
