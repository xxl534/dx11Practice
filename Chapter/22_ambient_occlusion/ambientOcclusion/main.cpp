...
private:
	void BuildVertexAmbientOcclusion(std::vector<Vertex::AmbientOcclusion>& vertices, const std::vector<UINT>& indices);
...
void AmbientOcclusionApp::BuildSkullGeometryBuffers()
{
	...
	for (UINT i = 0; i < vcount; ++i)
	{
		...
		vertices[i].AmbientAccess = 0.f;
		vertices[i].Tex = XMFLOAT2(0.f, 0.f);
	}
