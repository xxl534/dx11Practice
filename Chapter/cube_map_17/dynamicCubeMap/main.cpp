...
void CubeMapApp::DrawScene(const Camera* camera, bool drawCenterSphere)
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::BasicFX->SetTexTransform(XMMatrixScaling(6.f, 8.f, 1.f));
	...
	if (drawCenterSphere)
	{
		...
		for (UINT p = 0; p < techDesc.Passes; ++p)
		{
			...
			md3dImmediateContext->PSSetShaderResources(0, 2, srv);
		}
	}
	...
	Effects::BasicFX->SetCubeMap(NULL);
}
...
void CubeMapApp::BuildDynamicCubeMapViews()
{
	...
	HR(md3dDevice->CreateTexture2D(&texDesc, NULL, &cubeTex));
	...
	HR(md3dDevice->CreateTexture2D(&depthTexDesc, NULL, &depthTex));
