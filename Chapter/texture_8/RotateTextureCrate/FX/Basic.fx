...
cbuffer cbPerObject
{
	...
	float4x4 gDiffuseTexTransform;
	float4x4 gAlphaTexTransform;
	Material gMaterial;
}; 
...
VertexOut VS(VertexIn vin)
{
	...
	vout.TexDiffuse = mul(float4(vin.Tex, 0.0f, 1.0f), gDiffuseTexTransform).xy;
	vout.TexAlpha = mul(float4(vin.Tex, 0.0f, 1.0f), gAlphaTexTransform).xy;
...
float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure) : SV_Target
{
	...
	{
		// Sample texture.
		texColor = gDiffuseMap.Sample( samAnisotropic, pin.TexDiffuse );
		texColor *= gAlphaMap.Sample( samAnisotropic, pin.TexAlpha );
	}
