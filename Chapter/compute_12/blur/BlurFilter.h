...
public:
	...
	void SetWeights(const float weight[9]);

	///<summary>
	///The width and height should match the dimensions of the input texture to blur,
	///It is OK to call Init() again to reinitialize the blur filter with ad different dimension or format.
	///</summary>
	void Init(ID3D11Device* device, UINT width, UINT height, DXGI_FORMAT format);

	///<summary>
	///Blurs the input texture blurCount times, Note that this modifies the input texture, not a copu of it.
	///</summary>
