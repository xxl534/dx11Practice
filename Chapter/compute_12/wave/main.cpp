...
private:
	...
	ID3D11ShaderResourceView* mCurrWaveSRV;
	ID3D11UnorderedAccessView*	mCurrWaveUAV;

	ID3D11ShaderResourceView* mNextWaveSRV;
	ID3D11UnorderedAccessView*	mNextWaveUAV;
...
BlurApp::~BlurApp()
{
	...
	ReleaseCOM(mCurrWaveSRV);
	ReleaseCOM(mCurrWaveUAV);
	ReleaseCOM(mNextWaveSRV);
	ReleaseCOM(mNextWaveUAV)
...
void BlurApp::UpdateWave(float dt)
{
...
void BlurApp::BuildWaveGeometryBuffers()
{
	...
	vdb.ByteWidth = sizeof(Vertex::Basic32) * mWaveCol * mWaveRow;
	vdb.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vdb.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	vdb.MiscFlags = 0;
	HR(md3dDevice->CreateBuffer(&vdb, 0, &mWavesVB));

	UINT triangleCount = (mWaveCol - 1)*(mWaveRow - 1) * 2;
	std::vector<UINT> indices(3 * triangleCount);

	int k = 0;
	for (UINT i = 0; i < mWaveRow - 1; ++i)
	{
		for (UINT j = 0; j < mWaveCol - 1; ++j)
		{
			indices[k] = i*mWaveCol + j;
			indices[k + 1] = i*mWaveCol + j + 1;
			indices[k + 2] = (i + 1)*mWaveCol + j;

			indices[k + 3] = (i + 1)*mWaveCol + j;
			indices[k + 4] = i*mWaveCol + j + 1;
			indices[k + 5] = (i + 1)*mWaveCol + j + 1;
...
void BlurApp::BuildWaveViews()
{
	DXGI_FORMAT format = DXGI_FORMAT_A8_UNORM;
	D3D11_TEXTURE2D_DESC texDesc;
	texDesc.Width = mWaveCol;
	texDesc.Height = mWaveRow;
	texDesc.MipLevels = 1;
	texDesc.ArraySize = 1;
	...
	ID3D11Texture2D* texCurr = NULL;
	HR(md3dDevice->CreateTexture2D(&texDesc, NULL, &texCurr));
	ID3D11Texture2D* texNext = NULL;
	HR(md3dDevice->CreateTexture2D(&texDesc, NULL, &texNext));
	...
	HR(md3dDevice->CreateShaderResourceView(texCurr, &srvDesc, &mCurrWaveSRV));
	HR(md3dDevice->CreateShaderResourceView(texNext, &srvDesc, &mNextWaveSRV));

	D3D11_UNORDERED_ACCESS_VIEW_DESC uavDesc;
	uavDesc.Format = format;
	uavDesc.ViewDimension = D3D11_UAV_DIMENSION_TEXTURE2D;
	uavDesc.Texture2D.MipSlice = 0;
	HR(md3dDevice->CreateUnorderedAccessView(texCurr, &uavDesc, &mCurrWaveUAV));
	HR(md3dDevice->CreateUnorderedAccessView(texNext, &uavDesc, &mNextWaveUAV));

	ReleaseCOM(texCurr);
	ReleaseCOM(texNext);
}

void BlurApp::InitWave()
{
