...
{
	BoundingSphere() :Center(0.f, 0.f, 0.f), Radius(0.f) {}
...
bool MeshViewApp::Init()
{
	...
	mSsao = new Ssao(md3dDevice, md3dImmediateContext, mClientWidth, mClientHeight, mCamera->GetFovY(), mCamera->GetFarZ());
	...
	mCharactorModel = new SkinnedModel(md3dDevice, mTexMgr, "Models/Soldier.m3d", L"Textures/");
	SkinnedModelInstance charactorInstance1;
	SkinnedModelInstance charactorInstance2;
	charactorInstance1.Model = mCharactorModel;
	charactorInstance2.Model = mCharactorModel;
	charactorInstance1.TimePos = 0.f;
	charactorInstance2.TimePos = 0.f;
	charactorInstance1.ClipName = "Take1";
	charactorInstance2.ClipName = "Take1";
	charactorInstance1.FinalTransforms.resize(mCharactorModel->SkinnedData.BoneCount());
	charactorInstance2.FinalTransforms.resize(mCharactorModel->SkinnedData.BoneCount());

	modelScale = XMMatrixScaling(0.05f, 0.05f, -0.05f);
	modelRot = XMMatrixRotationY(MathHelper::Pi);
	modelOffset = XMMatrixTranslation(-3.0f, 1.0f, -5.0f);
	XMStoreFloat4x4(&charactorInstance1.World, modelScale*modelRot*modelOffset);

	modelOffset = XMMatrixTranslation(3.0f, 1.0f, -5.0f);
	XMStoreFloat4x4(&charactorInstance2.World, modelScale*modelRot*modelOffset);

	mSkinnedModelInstances.push_back(charactorInstance1);
	mSkinnedModelInstances.push_back(charactorInstance2);
	//
...
void MeshViewApp::UpdateScene(float dt)
{
	...
	for (UINT i = 0; i < mSkinnedModelInstances.size(); ++i)
	{
		mSkinnedModelInstances[i].Update(dt);
	}
...
void MeshViewApp::DrawScene()
{
	...
	//
	ID3D11RenderTargetView* rt[1] = { mRenderTargetView };
	md3dImmediateContext->OMSetRenderTargets(1, rt, mDepthStencilView);
	md3dImmediateContext->RSSetViewports(1, &mScreenViewport);

	md3dImmediateContext->ClearRenderTargetView(mRenderTargetView, reinterpret_cast<const float*>(&Colors::Silver));
	//We already laid down scene depth to the depth buffer in the Normal/Depth map pass,
	//so we can set the depth comparison test to EQUALS. This prevents any overdraw in this
	//rendering pass, as only the nearest visible pixels will pass the depth comparison test.
	//md3dImmediateContext->OMSetDepthStencilState(RenderStates::LessEqualDSS, 0);

	//But in case of drawing displacement map,the depth stencil state above is invalid because ssao is drawn without
	//displacement map and errors exist in vertex's world coordinate.
	md3dImmediateContext->ClearDepthStencilView(mDepthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.f, 0);
	...
	ID3DX11EffectTechnique* skinnedTech = Effects::NormalMapFX->Light3TexSkinnedTech;
	...
	skinnedTech->GetDesc(&techDesc);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		ID3DX11EffectPass* pass = skinnedTech->GetPassByIndex(p);
		for (UINT modelIndex = 0; modelIndex < mSkinnedModelInstances.size(); ++modelIndex)
		{
			world = XMLoadFloat4x4(&mSkinnedModelInstances[modelIndex].World);
			worldInvTranspose = MathHelper::InverseTranspose(world);
			worldViewProj = world*view*proj;
			...
			Effects::NormalMapFX->SetBoneTransforms(
				&mSkinnedModelInstances[modelIndex].FinalTransforms[0],
				mSkinnedModelInstances[modelIndex].FinalTransforms.size());

			for (UINT subset = 0; subset < mSkinnedModelInstances[modelIndex].Model->SubsetCount; ++subset)
			{
				Effects::NormalMapFX->SetMaterial(mSkinnedModelInstances[modelIndex].Model->Mat[subset]);
				Effects::NormalMapFX->SetDiffuseMap(mSkinnedModelInstances[modelIndex].Model->DiffuseMapSRV[subset]);
				Effects::NormalMapFX->SetNormalMap(mSkinnedModelInstances[modelIndex].Model->NormalMapSRV[subset]);

				skinnedTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
				mSkinnedModelInstances[modelIndex].Model->ModelMesh.Draw(md3dImmediateContext, subset);
			}
		}
	}
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		ID3DX11EffectPass* pass = alphaClippedTech->GetPassByIndex(p);
...
void MeshViewApp::DrawSceneToSsaoNormalDepthMap()
{
	...
	ID3DX11EffectTechnique* skinnedTech = Effects::SsaoNormalDepthFX->NormalDepthSkinnedTech;
...
void MeshViewApp::DrawSceneToShadowMap()
{
	...
	Effects::BuildShadowMapFX->SetEyePosW(mCamera->GetPosition());
	...
	ID3DX11EffectTechnique* skinnedTech = Effects::BuildShadowMapFX->BuildShadowMapSkinnedTech;
