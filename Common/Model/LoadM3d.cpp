...
bool M3DLoader::LoadM3d(const std::string& filename, std::vector<Vertex::BasicTangentSkinned>& vertices, std::vector<USHORT>& indices, std::vector<MeshGeometry::Subset>& subsets, std::vector<M3dMaterial>& mats, SkinnedData& skinInfo)
{
	std::ifstream fin(filename);
...
void M3DLoader::ReadVertices(std::ifstream& fin, UINT numVertices, std::vector<Vertex::BasicTangent>& vertices)
{
	std::string ignore;
	...
	for (UINT i = 0; i < numVertices; ++i)
	{
		...
		fin >> ignore >> vertices[i].Tangent.x >> vertices[i].Tangent.y >> vertices[i].Tangent.z >> ignore;
...
void M3DLoader::ReadSkinnedVertices(std::ifstream& fin, UINT numVertices, std::vector<Vertex::BasicTangentSkinned>& vertices)
{
	std::string ignore;
	vertices.resize(numVertices);
...
void M3DLoader::ReadAnimationClips(std::ifstream& fin, UINT numBones, UINT numAnimationClips, std::map<std::string, AnimationClip>& animations)
{
	std::string ignore;
